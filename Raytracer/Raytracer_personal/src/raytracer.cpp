...
int main(int argc, char *argv[]) {
    ...
    auto checkered = make_shared<checkers>(0.32, color(.2, .3, .1), color(.9, .9, .9));
    world.add(make_shared<sphere>(vec3(0, -1000, 0), 1000, checkered));
    ...
    auto dur = std::chrono::duration_cast<std::chrono::minutes>(stop - start);

    std::clog << "Runtime: " << dur.count() << " minutes" << std::flush;
